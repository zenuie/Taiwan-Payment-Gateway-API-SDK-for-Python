...
def trigger_payment():
    ...
    logging.info("\n--- Triggering Example: pay_with_atm ---")
    try:
        adapter: EcpayAdapter = factory.get_adapter("ecpay")
        order_id = generate_order_id("ATM")
        atm_input = EcpayAtmPaymentInput(
            order_id=order_id,
            amount=250,
            currency="TWD",
            details="Flask ATM Test Payment",
            return_url="https://your-publicly-accessible-domain.com/callback/ecpay_return",  # MUST be public
            payment_info_url="https://your-publicly-accessible-domain.com/callback/ecpay_payment_info",  # MUST be public
            expire_date=5,
        )
        output = adapter.pay_with_atm(atm_input)
