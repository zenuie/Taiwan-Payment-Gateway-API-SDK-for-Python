...
def trigger_payment():
    ...
    logging.info("\n--- Triggering Example: pay_with_credit_periodic ---")
    try:
        adapter: EcpayAdapter = factory.get_adapter("ecpay")
        order_id = generate_order_id("PERIOD")
        periodic_amount = 75
        credit_input = EcpayCreditPaymentInput(
            order_id=order_id,
            amount=periodic_amount,  # Must equal period_amount
            currency="TWD",
            details="Flask Periodic Test (Every 2 Months)",
            return_url="https://your-publicly-accessible-domain.com/callback/ecpay_return",  # MUST be public
            period_amount=periodic_amount,
            period_type="M",
            frequency=2,
            exec_times=4,
            period_return_url="https://your-publicly-accessible-domain.com/callback/ecpay_periodic_return",  # MUST be public
        )
