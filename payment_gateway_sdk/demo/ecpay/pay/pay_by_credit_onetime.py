...
def trigger_payment():
    ...
    logging.info("\n--- Triggering Example: pay_with_credit_onetime ---")
    try:
        adapter: EcpayAdapter = factory.get_adapter("ecpay")
        order_id = generate_order_id("CREDIT1")
        credit_input = EcpayCreditPaymentInput(
            order_id=order_id,
            amount=150,
            currency="TWD",
            details="Flask One-time Credit Card Test",
            return_url="https://your-publicly-accessible-domain.com/callback/ecpay_return",  # MUST be public
            client_redirect_url="https://your-publicly-accessible-domain.com/payment_success",  # Optional
            need_extra_paid_info="Y",
            client_back_url="https://your-publicly-accessible-domain.com/cart",
        )
