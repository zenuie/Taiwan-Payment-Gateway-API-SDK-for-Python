...
from payment_gateway_sdk.gateways.ecpay import EcpayAdapter, EcpayBarcodePaymentInput
...
def trigger_payment():
    ...
    logging.info("\n--- Triggering Example: pay_with_barcode ---")
    try:
        adapter: EcpayAdapter = factory.get_adapter("ecpay")
        order_id = generate_order_id("BARCODE")
        barcode_input = EcpayBarcodePaymentInput(
            order_id=order_id,
            amount=450,
            currency="TWD",
            details="Flask Barcode Test Payment",
            return_url="https://your-publicly-accessible-domain.com/callback/ecpay_return",  # MUST be public
            client_redirect_url_for_info="https://your-publicly-accessible-domain.com/show_barcode_info",  # Optional
            store_expire_date=10,
        )
        output = adapter.pay_with_barcode(barcode_input)
