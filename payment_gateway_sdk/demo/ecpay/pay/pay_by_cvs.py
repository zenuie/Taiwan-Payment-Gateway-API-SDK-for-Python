...
from payment_gateway_sdk.gateways.ecpay import EcpayAdapter, EcpayCvsPaymentInput
...
def trigger_payment():
    ...
    logging.info("\n--- Triggering Example: pay_with_cvs ---")
    try:
        adapter: EcpayAdapter = factory.get_adapter("ecpay")
        order_id = generate_order_id("CVS")
        cvs_input = EcpayCvsPaymentInput(
            order_id=order_id,
            amount=333,
            currency="TWD",
            details="Flask CVS Test Payment",
            return_url="https://your-publicly-accessible-domain.com/callback/ecpay_return",  # MUST be public
            client_redirect_url_for_info="https://your-publicly-accessible-domain.com/show_cvs_info",  # Optional
            store_expire_date=1440 * 5,
            desc_1="Flask Item 1",
            desc_2="Flask Item 2",
        )
        output = adapter.pay_with_cvs(cvs_input)
